...
pub mod pallet {
	...
	pub type Memberships<T: Config> =
		StorageDoubleMap<_, Identity, T::Hash, Identity, T::AccountId, (), OptionQuery>;

	#[pallet::type_value]
	pub fn DefaultOpinion() -> Opinion {
		Opinion::Neutral
	}

	/// Storage for inte-user relations
	/// keys:
	/// [Tile ID, about Whom, Who stated opinion]
	/// value:
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn join(origin: OriginFor<T>, tile_id: T::Hash) -> DispatchResult {
